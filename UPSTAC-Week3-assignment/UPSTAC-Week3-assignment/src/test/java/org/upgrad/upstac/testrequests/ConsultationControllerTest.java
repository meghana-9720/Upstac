...
class ConsultationControllerTest {
    ...
    public void calling_assignForConsultation_with_valid_test_request_id_should_throw_exception(){
        ...
        ResponseStatusException obj=assertThrows(ResponseStatusException.class,()->{consultationController.assignForConsultation(InvalidRequestId);});
    ...
    public void calling_updateConsultation_with_valid_test_request_id_should_update_the_request_status_and_update_consultation_details(){
        ...
        CreateConsultationRequest req=getCreateConsultationRequest(testRequest);
        ...
        consultationController.updateConsultation(testRequest1.requestId,req);
    ...
    public void calling_updateConsultation_with_invalid_test_request_id_should_throw_exception(){
        ...
        ResponseStatusException obj=assertThrows(ResponseStatusException.class,()->{consultationController.updateConsultation(-34L,req);});
    ...
    public void calling_updateConsultation_with_invalid_empty_status_should_throw_exception(){
        ...
        CreateConsultationRequest req=getCreateConsultationRequest(testRequest);
        req.setSuggestion(null);

        // Create an object of ResponseStatusException . Use assertThrows() method and pass updateConsultation() method
        // of consultationController with request Id of the testRequest object and the above created object as second parameter
        //Refer to the TestRequestControllerTest to check how to use assertThrows() method
        ResponseStatusException obj=assertThrows(ResponseStatusException.class,()->{consultationController.updateConsultation(testRequest.requestId,req);});
    ...
    public CreateConsultationRequest getCreateConsultationRequest(TestRequest testRequest) {
        ...
        result.setTemperature("88F");
        result.setOxygenLevel("Normal");
        result.setHeartBeat("72");
        result.setComments("Ok");
        CreateConsultationRequest req=getCreateConsultationRequest(testRequest);
        if(result.getResult()==TestStatus.NEGATIVE){

            req.setSuggestion(DoctorSuggestion.NO_ISSUES);
        }
        else {

            req.setSuggestion(DoctorSuggestion.HOME_QUARANTINE);
        }
